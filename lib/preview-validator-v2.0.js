...
const runInBrowser = async function (path) {
  ...
  page.on("request", function (request) {
    ...
    if (
      domainWhitelist.indexOf(domain) === -1 &&
      requestUrl.substr(0, 5) !== "data:"
    ) {
  ...
  await page.goto(`http://localhost:${port}/${previewHtml}`, {
  ...
  const formatCount = await page.evaluate(() => {
    return document.querySelector(".dcbde-creative-size-selector select")
      .childElementCount;
  });
  ...
  for (let i = 1; i <= formatCount; ++i) {
    const formatNameValue = await page.evaluate((num) => {
      return {
        name: document.querySelector(
          ".dcbde-creative-size-selector select option:nth-child(" + num + ")"
        ).label,
    ...
    printStatus(
      `Testing format "${formatNameValue.name}" (${i}/${formatCount})...`
    );
    await page.select(
      ".dcbde-creative-size-selector select",
