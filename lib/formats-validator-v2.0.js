...
const sizeIsCorrect = function (path, width, height) {
  ...
  return (
    Math.abs(size.width - width) < epsilon &&
    Math.abs(size.height - height) < epsilon
  );
};
...
const validateSize = function (path, size) {
  if (size == null || typeof size !== "object") {
    ...
    size.type !== "ADAPTIVE" &&
    size.type !== "FIXED" &&
    size.type !== "RESPONSIVE" &&
    size.type !== "PREVIEW"
  ) {
    printError(
      `"${size.name}" format's property 'type' is "${size.type}" which is not supported!`
    );
    ...
    printError(
      `"${size.name}" format's property 'index' is missing or not a string.`
    );
    ...
    printError(
      `"${size.name}" format's 'fallback' object is missing 'static' property.`
    );
  } else if (typeof size.fallback.static !== "string") {
    printError(
      `"${size.name}" format's fallback 'static' property is not a string.`
    );
  } else if (!fileExists.sync(path + "/" + size.fallback.static)) {
    printError(`"${size.name}" format's fallback file is not found!`);
  } else if (
    !new RegExp("^.*.(png|jpg|gif)$").test(size.fallback.static.toLowerCase())
  ) {
    printError(
      `"${size.name}" format's fallback file must be a png, jpg or gif.`
    );
  } else if (
    !size.width.toString().includes("%") &&
    !size.height.toString().includes("%") &&
    size.width.toString() !== "1" &&
    size.height.toString() !== "1" &&
    size.width.toString() !== "0" &&
    size.height.toString() !== "0" &&
    (size.type === "STATIC" || size.type === "FIXED") &&
    !sizeIsCorrect(path + "/" + size.fallback.static, size.width, size.height)
  ) {
    printError(
      `"${size.name}" format's fallback file doesn't match specified dimensions (${size.width}x${size.height})!`
    );
  } else {
