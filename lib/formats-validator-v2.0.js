...
const validateSize = function(path, size) {
	...
	else if (size.type !== 'STATIC' && size.type !== 'ADAPTIVE' && size.type !== 'FIXED' && size.type !== 'RESPONSIVE' && size.type !== 'PREVIEW') {
	...
	else if (!size.width.toString().includes("%") && !size.height.toString().includes("%") && size.width.toString() !== "1" && size.height.toString() !== "1" && size.width.toString() !== "0" && size.height.toString() !== "0" && (size.type === 'STATIC' || size.type === 'FIXED') && !sizeIsCorrect(path + '/' + size.fallback.static, size.width, size.height)) {
