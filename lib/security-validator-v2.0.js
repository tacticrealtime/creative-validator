...
const formatError = function(element, messages, path) {
	...
	if (messages == null || messages.message == null || typeof messages.message !== 'string') {
		printError('Error message is not found');
	}
	else if (messages.severity == 2) {
		if (messages.message.indexOf('localStorage') !== -1) {
			printError(`Prohibited use of 'localStorage' is found at "${element.filePath.replace(path,".")}" line ${messages.line} column ${messages.column}`);
		}
		else if (messages.message.indexOf('sessionStorage') !== -1) {
			printError(`Prohibited use of 'sessionStorage' is found at "${element.filePath.replace(path,".")}" line ${messages.line} column ${messages.column}`);
		}
		else if (messages.message.indexOf('document.cookie') !== -1) {
			printError(`Prohibited use of 'document.cookie' is found at "${element.filePath.replace(path,".")}" line ${messages.line} column ${messages.column}`);
		}
		else {
			printError(`"${messages.message}" is found at "${element.filePath.replace(path,".")}" line ${messages.line} column ${messages.column}`);
		}
	}
};
...
const validatePath = function(path, debug) {
	...
	if (Array.isArray(report.results)) {
		report.results.forEach(function(element) {
			if (typeof element.messages !== 'undefined' && element.messages.length > 0 && Array.isArray(element.messages)) {
